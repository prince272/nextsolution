...
export const CookiesProvider: FC<{
  ...
  const insertedHTML = useContext<ServerInsertedHTMLHook | null>(ServerInsertedHTMLContext as never);

  const cookies = useMemo((): Cookies => {
    const map: Partial<Record<string, string>> = Object.fromEntries(value.map((c) => [c.name, c.value]));
...
const deserializeCookieAttributes = (attributes: SerializedValue<CookieAttributes>): CookieAttributes => ({
  ...attributes,
