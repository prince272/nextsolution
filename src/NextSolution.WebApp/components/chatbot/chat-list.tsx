...
export const ChatList: FC<ChatListProps> = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    if (chats.offset == null) loadData(0);
  }, []);
  ...
  return (
    <>
      <GroupedVirtuoso
        ...
        endReached={() => {
          if (chats.next) loadData(chats.next);
        }}
        itemContent={(index) => {
            ...
            return (
              <Render switch={chatsStatus.action == "loading" ? "loading" : chatsStatus.error ? "error" : "blank"}>
                <div key="error" className="flex h-24 flex-col items-center justify-center space-y-2 text-center">
                  <div className="p-2 text-sm text-foreground-500">{getErrorMessage(chatsStatus.error)}</div>
                  <Button
                    variant="light"
                    color="primary"
                    onPress={() => {
                      loadData(chats.next || 0);
                    }}
                  >
                    Reload
                  </Button>
      ...
      />
      <Button
        startContent={<DeleteIcon className="pointer-events-none h-5 w-5 flex-shrink-0 text-xl text-danger" type="outlined" />}
        className="mx-3 my-3 h-11 justify-start px-2"
        variant="light"
        as={NextLink}
        href={queryString.stringifyUrl({ url: pathname, query: { dialogId: "delete-all-chats" } })}
...
export const ChatListComponents = {
  ...
  Group: forwardRef<HTMLDivElement, ComponentPropsWithoutRef<"div">>(({ className, ...props }, ref) => {
    return <div className={cn("bg-background py-2 pl-3 text-xs font-semibold text-primary", className)} {...props} ref={ref} />;
  }),
  // eslint-disable-next-line react/display-name
