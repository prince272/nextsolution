...
export const convertToCssVariables = (obj: { [key: string]: any }, prefix: string = "", suffix: string = ""): { [key: string]: string } => {
  const convertedObject: { [key: string]: string } = {};

  const convert = (obj: { [key: string]: any }, parentKey: string = "") => {
    for (const [key, value] of Object.entries(obj)) {
      const newKey = parentKey ? `${parentKey}-${key.replace(/([A-Z])/g, "-$1").toLowerCase()}` : `${prefix}${key.replace(/([A-Z])/g, "-$1").toLowerCase()}${suffix}`;
